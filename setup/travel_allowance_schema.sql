...
BEGIN
  FOR policy_record IN SELECT tablename, policyname FROM pg_catalog.pg_policies
    WHERE schemaname = 'public' AND tablename IN ('travel_allowance_requests', 'employee_details')
  LOOP
...
SET search_path = ''
AS $$
  SELECT empid FROM public.profiles WHERE auth.uid() IS NOT NULL AND id = auth.uid();
$$;
